...
// Switches to task time when the clock reaches 0:00.
function countDown() {
  ...
  if (time === '0') {
    ...
    if ($('#notify').is(':checked')) {
      notify('Task Timer complete! Sessions completed: ' + sessionCount);
    }

    $('#sessionCount').html('<span style="color:red">&#x1f345</span> ' + sessionCount);
    startTimer('task');
...
function showTime(seconds, id) {
  ...
  if (id == 'clockTime') {
    ...
    //task times shown as H:MM, MM, or M.
    if (displayHours === 0) {
      time = displayMinutes;
